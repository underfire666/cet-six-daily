...
V5 已正式开放“阅读”专项（等待人工验收）：每日固定 3 篇阅读、每篇完成展示正确率与进度、生词点击查义并加入统一生词本（跨专项去重）、当日 3 篇完成即阅读专项完成。听力、翻译、写作仍为“敬请期待”。后续开发须由用户明确提出，不自行开始 V6。

### V5 实现与自测记录（2026-09-18）

- 新增 6 篇原创 B2 英文文章（250–450 词、每篇 4 题、7 个查词条目），每日任务固定 3 篇且同日结果稳定。
- 阅读答题复用 V2 通用答题状态机（一次重试、末尾复测、AI 提示、解析）；完成页展示正确率、独立答对、需加强、本篇生词与 XP。
...
- 完成一篇展示正确率（独立答对 / 总题数）、需加强题数、本篇生词数与 XP；XP = 基础 15 + 每题表现分（首轮答对 2、二次答对或 AI 提示后答对 1、其余 0）。
