...
保留第一版学习首页，新增可完整作答的「每日总关卡」与通用答题系统。使用本地 Mock 数据，面向本地浏览器演示。
...
| `/practice/[module]` | 原有五项专项占位页；完成页也可选择进入 |
| `/ai`、`/me` | 原有占位页 |
...
- `profile`：首次使用日期、完成过的学习日期、按实际完成日期登记的奖励。
...
题目、AI 提示、初始用户、历史计划和考试目标仍为 Mock。没有真实 AI API、数据库、登录、题库服务或复杂听力播放器；五项专项、AI 页和个人页仍为占位功能。
