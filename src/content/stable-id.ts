 ...
 *   REAL PAPER（真实 / past_exam 试卷）namespace：
 ...
 *
 * 两个 namespace 结构上可区分（第 2 段 `fixture` 为 fixture namespace 保留字）：
 * - REAL paper 永远不可能撞上 FIXTURE paper 的 stable ID / identity；
 * - 未来导入真实 2025-12 set1 时，与 synthetic fixture 无任何 collision；
 * - 交叉校验：fixture=true 必须用 fixture namespace；real/past_exam 禁止用 fixture namespace。
 *
...
/** REAL Paper 稳定 ID：cet6:2025-12:set1 */
...
/** 通用追加段：在任意合法 paper/section 前缀上追加子段（REAL 与 FIXTURE 共用）。 */
export function extendStableId(base: string, ...segments: string[]): string {
  if (!segments.length) return base;
  return `${base}:${segments.join(":")}`;
}

/** Section 稳定 ID：cet6:2025-12:set1:reading / cet6:fixture:synthetic-001:reading */
...
/** 校验 Paper 树 stable ID 格式（REAL 与 FIXTURE namespace 均合法）。 */
export function isValidStableId(id: string, kind: StableIdKind): boolean {
  if (typeof id !== "string" || !id.length) return false;
  if (kind === "paper") return REAL_PAPER_RE.test(id) || FIXTURE_PAPER_RE.test(id);
  if (kind === "section") return SECTION_REAL_RE.test(id) || SECTION_FIXTURE_RE.test(id);
  // group/question/asset：section 前缀（前 4 段）+ :<…> 后缀
  const prefix = id.split(":").slice(0, 4).join(":");
  return (SECTION_REAL_RE.test(prefix) || SECTION_FIXTURE_RE.test(prefix)) && id.split(":").length >= 5;
}

export type StableIdNamespace = "real" | "fixture" | "invalid";

/**
 * 判别 stable ID 属于哪个 namespace（前 3 段 = paper 前缀段）：
 * - "real"：真实 / past_exam 试卷（cet6:<year>-<session>:set<N>…）
